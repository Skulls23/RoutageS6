...
public class Metier
{
    ...
    public static String getTableRoutage(Graph graph)
    {
        String ret = "";
        ...
        for(int i=0; i<graph.getNodeCount(); i++)
        {
            ...
            for(int j=0; j<graph.getNodeCount(); j++)
            {
                ...
                ret += "\t" + pointDebut.getId();

                Metier.setupDijkstra(graph, pointDebut.getId());

                HashMap<Double, String> mapPoidsVoisinVersFinal = new HashMap<>(); //lie un poids de chemin a un nom de noeud
                ...
                for (int k = 0; k < tabVoisin.length; k++)
                {
                    ...
                    mapPoidsVoisinVersFinal.put(dijkstra.getPathLength(pointFin) + tabPoidsVersVoisin[k], ((Node)tabVoisin[k]).getId());
                }
                TreeMap<Double, String> treeMap = new TreeMap<>(mapPoidsVoisinVersFinal); //tri la hashmap par poids de chemin
                System.out.println(pointFin.getId() + " : " + treeMap);
            }
        }
        return "";
    }
